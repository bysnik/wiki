...
Будем предполагать, что у Вас на клиенте под учётной записью пользователя уже создать открытый ключ `~/.ssh/id_rsa.pub`, который клиент будет использовать для доступа к Вашему Git-репозиторию.
...
Теперь каждый раз, когда нужно будет создать новый проект/репозиторий, необходимо будет выполнять следующие шаги yна сервере:
...
Теперь можно клонировать следующим образом:
```bash
git clone git://<git_server_ip_address>/my_project.git # без /srv/git
```
...
Так же необходимо установить Unix пользователя и группу для каталога `/srv/git` в значение `apache2`, чтобы позволить веб-серверу читать из и писать в репозитории, потому что процесс Apache, запускающий CGI скрипт, работает от имени этого пользователя:
```bash
chgrp -R apache2 /srv/git
```
