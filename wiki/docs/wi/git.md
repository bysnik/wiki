...
<ImageZoom src="https://cdn.thenewstack.io/media/2021/08/4ce8bc99-git.png" />
...
<ImageZoom src="https://habrastorage.org/getpro/habr/post_images/a91/fe3/bda/a91fe3bda5ec9f80d35d3c2b6b6f1ddd.png" />
...
<ImageZoom src="/img/gitwebtest.png" />
...
<ImageZoom src="https://git-lfs.com/images/graphic.gif" />
...
## Cинхронизация форка с upstream (оригинальным репозиторием)

### 1. Добавьте upstream (если ещё не добавлен)
```bash
git remote add upstream https://github.com/владелец/репозиторий.git
```
Проверьте:
```bash
git remote -v
...
### 2. Скачайте свежие изменения из upstream
```bash
git fetch upstream
```

### 3. Переключитесь на свою основную ветку (обычно `main` или `master`)
```bash
git checkout main
```

### 4. Слейте изменения (выберите один способ)

**Способ А – слияние (merge) – проще и безопаснее**
```bash
git merge upstream/main
```
При конфликтах переходите к п.5.

**Способ Б – перебазирование (rebase) – даёт чистую историю**
```bash
git rebase upstream/main
```
При конфликтах процесс остановится – разрешаете и продолжаете (`git add . && git rebase --continue`). Если ошиблись – прервите (`git rebase --abort`).

::: details Небольшая НЕЙРОподсказка
## Разница между merge и rebase

Оба инструмента объединяют изменения из одной ветки в другую, но делают это по‑разному и с разными последствиями для истории.
...
**Как работает**  
Git создаёт новый **коммит слияния** (merge commit), который имеет двух родителей: последний коммит вашей ветки и последний коммит ветки, которую вы вливаете. История остаётся *полной* и отражает реальный ход разработки – все ветки видны как отдельные линии, которые в какой-то момент пересекаются.

```
      A---B---C   (ваша ветка)
     /         \
    D---E---F---G   (upstream) + merge commit G
```

**Плюсы**  
- Не переписывает историю – безопасно для общих веток.  
- Простой откат (`git revert <merge-commit>`).  
- Идеально подходит для публичных репозиториев, где несколько разработчиков работают над одной веткой.

**Минусы**  
- История становится «ветвистой» и может выглядеть запутанно, особенно при частых слияниях.  
- Лишний коммит слияния, который не несёт смысловой нагрузки.
...
**Как работает**  
Git берёт ваши коммиты и **переносит их** на вершину целевой ветки, как будто вы начали работу с актуальной версии. При этом создаются новые коммиты с новыми хешами, а старые «забываются». История становится *линейной* и чистой.

```
Ваши коммиты до rebase:
      A---B---C   (ваша ветка)
     /
    D---E---F   (upstream)

После rebase (перенос на upstream):
    D---E---F---A'---B'---C'   (ваша ветка)
```

**Плюсы**  
- Линейная, легко читаемая история без лишних слияний.  
- Упрощает поиск ошибок (git bisect).  
- Убирает конфликты по частям – каждый коммит применяется последовательно.

**Минусы**  
- Переписывает историю – **опасно** для публичных веток, где уже есть коллеги. После ребейза ваша ветка становится несовместимой с их версией, и приходится делать `push --force`, что может сломать работу команды.  
- При возникновении конфликтов их нужно разрешать для *каждого* вашего коммита по отдельности (может быть утомительно).  
- Сложнее откатить (придётся искать старые хеши).
...
|----------|--------------|
| Вы работаете **один** в своей ветке (форк, локальная ветка) | Можно использовать **rebase** – история будет чистой. |
| Ветка **публичная** (её уже кто-то использует) | **merge** – безопаснее, не нарушает работу других. |
| Вы хотите сохранить историю слияний, чтобы видеть, когда и откуда брались изменения | **merge**. |
| Вам нужна максимально простая линейная история для обзора кода | **rebase** (но только перед тем, как отправить PR, обычно делают rebase на актуальный мастер). |

---

**Типичный рабочий процесс в open‑source**

1. Вы создали форк, сделали коммиты.  
2. Перед отправкой Pull Request вы делаете `git fetch upstream` и **rebase** свою ветку на актуальный `upstream/main` – чтобы PR можно было влить без конфликтов и с линейной историей.  
3. Но если вы уже отправили PR и другие разработчики начали обсуждать его, rebase лучше не делать – используйте merge.
:::

### 5. Разрешение конфликтов
- Git покажет список конфликтующих файлов.
- Откройте каждый, найдите маркеры `<<<<<<<`, `=======`, `>>>>>>>`.
- Отредактируйте код, оставив нужное, удалите маркеры.
- После правки добавьте файлы:
  ```bash
  git add .
  ```
- Если использовали **merge**, завершите коммитом:
  ```bash
  git commit -m "Merge upstream changes"
  ```
- Если использовали **rebase**, продолжите:
  ```bash
  git rebase --continue
  ```
  Повторяйте, пока все конфликты не разрешены.

### 6. Отправьте результат в свой форк на GitHub
- Если использовали **merge**:
  ```bash
  git push origin main
  ```
- Если использовали **rebase** (история изменилась):
  ```bash
  git push --force-with-lease origin main
  ```
  (никогда не используйте простой `--force` без крайней необходимости).

### 7. Проверьте
Обновите страницу своего форка на GitHub – теперь он содержит и ваши доработки, и новые изменения из upstream.
...
- Посмотреть различия: `git diff HEAD upstream/main -- файл`
- Сделать резервную копию ветки: `git checkout -b my-backup`