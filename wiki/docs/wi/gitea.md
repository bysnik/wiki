...
Затем просто запустите service fail2ban restart, чтобы применить изменения. Вы можете проверить, принял ли fail2ban вашу конфигурацию, с помощью service fail2ban status.
...
```
Необходимо настроить параметры безопасности app.ini, чтобы обеспечить интерпретацию заголовков, а также списка IP-адресов и сетей, описывающих доверенные прокси-серверы (для получения дополнительной информации см. шпаргалку по настройке ).
```ini
...
Вы  можете создать токен ключа API через веб-интерфейс вашей установки Gitea: Настройки | Приложения | Создать новый токен.
