...
::: warning
Для работы этого трея нужно, чтобы у пользователя, который запускает сервис, были права sudo, потом их можно отключить. Иначе трей не запустится (нужно найти проблему и исправить!)
:::
