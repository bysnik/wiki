...
show_help() {
    echo "ВСЕ ДЕЙСТВИЯ НЕОБХОДИМО ВЫПОЛНЯТЬ ПОД ПОЛЬЗОВАТЕЛЕМ ROOT"
    echo "Использование: $0 <команда>"
    echo ""
    echo "Команды:"
    echo "  5.10                         - установка ядра 5.10.241-std-def-alt0.c9f.2. Необходим заранее подготовленный локальный репозиторий c9f2"
    echo "  6.12                         - установка ядра 6.12-6.12.41-alt0.c10f.2. Необходим заранее подготовленный локальный репозиторий c10f2"
    echo "  repo-c9f2                    - создание минимального репозитория c9f2 для установки ядра. ~3.5GB"
    echo "  repo-c10f2                   - создание минимального репозитория c10f2 для установки ядра. ~3.5GB"
    echo "  create-iso                   - подготовить iso-образ с репозиторием и данным скриптом. От ~3.GB до 8GB"
    echo "  burn-iso-interactive         - нарезать iso-образ в интерактивном режиме"
    echo "  help                         - показать эту справку"
    echo "  readme                       - показать подробную документацию"
    echo ""
...
readme() {
    echo "================================================================"
    echo "               УСТАНОВКА ЯДЕР ALT LINUX - ДОКУМЕНТАЦИЯ"
    echo "================================================================"
    echo ""
    echo "ОБЗОР ПРОЦЕССА:"
    echo "1. Выбор ядра для установки"
    echo "2. Скачивание минимального набора пакетов"
    echo "3. Подготовка локального репозитория"
    echo "4. Создание ISO образа с репозиторием и скриптом"
    echo "5. Запись ISO на USB/DVD носитель"
    echo "6. Установка ядра на целевой системе"
    echo ""
    echo "────────────────────────────────────────────────────────────────"
    echo "ШАГ 1: ВЫБОР ЯДРА И РЕПОЗИТОРИЯ"
    echo "────────────────────────────────────────────────────────────────"
    echo "Доступные варианты:"
    echo "  • 5.10.241-std-def-alt0.c9f.2  (репозиторий CF2)"
    echo "  • 6.12-6.12.41-alt0.c10f.2     (репозиторий c10f2)"
    echo ""
    echo "Команды для подготовки репозитория:"
    echo "  # ./kernels.sh repo-c9f2"
    echo "  # ./kernels.sh repo-c10f2"
    echo ""
    echo "────────────────────────────────────────────────────────────────"
    echo "ШАГ 2: СОЗДАНИЕ ISO ОБРАЗА"
    echo "────────────────────────────────────────────────────────────────"
    echo "После подготовки репозитория создайте ISO образ:"
    echo "  # ./kernels.sh create-iso"
    echo ""
    echo "Размер образа: от 3.5GB до 8GB в зависимости от выбранного ядра"
    echo "Образ будет содержать:"
    echo "  • Локальный репозиторий с пакетами ядра"
    echo "  • Данный скрипт kernels.sh"
    echo ""
    echo "────────────────────────────────────────────────────────────────"
    echo "ШАГ 3: ЗАПИСЬ НА НОСИТЕЛЬ"
    echo "────────────────────────────────────────────────────────────────"
    echo "Запись ISO образа на USB флешку или DVD:"
    echo "  # ./kernels.sh burn-iso-interactive"
    echo ""
    echo "Требования к носителю:"
    echo "  • USB флешка ≥4GB или DVD диск"
    echo "  • Все данные на носителе будут удалены!"
    echo ""
    echo "────────────────────────────────────────────────────────────────"
    echo "ШАГ 4: УСТАНОВКА НА ЦЕЛЕВОЙ СИСТЕМЕ"
    echo "────────────────────────────────────────────────────────────────"
    echo "1. Подключите носитель к компьютеру"
    echo "2. При необходимости смонтируйте устройство:"
    echo "   # mount /dev/sdX1 /mnt && cd /mnt"
    echo "3. Запустите установку:"
    echo "   # ./kernels.sh 5.10    # для ядра 5.10"
    echo "   # ./kernels.sh 6.12    # для ядра 6.12"
    echo ""
    echo "ПРОЦЕСС УСТАНОВКИ:"
    echo "  • Временное подключение локального репозитория"
    echo "  • Установка выбранного ядра"
    echo "  • Восстановление оригинальных настроек репозиториев"
    echo "  • Запрос на перезагрузку системы"
    echo ""
    echo "────────────────────────────────────────────────────────────────"
    echo "ВАЖНЫЕ ЗАМЕЧАНИЯ:"
    echo "────────────────────────────────────────────────────────────────"
    echo "✓ ВСЕ ДЕЙСТВИЯ НЕОБХОДИМО ВЫПОЛНЯТЬ ПОД ПОЛЬЗОВАТЕЛЕМ ROOT"
    echo "✓ Интернет не требуется, так как создается локальный репозиторий"
    echo "✓ Скрипт автоматически восстанавливает настройки apt-repo"
    echo ""
    echo "────────────────────────────────────────────────────────────────"
    echo "КОМАНДЫ ДЛЯ РУЧНОЙ ПРОВЕРКИ:"
    echo "────────────────────────────────────────────────────────────────"
    echo "Проверка содержимого ISO:"
    echo "  ls -la /your_mount_point/"
    echo ""
    echo "Проверка доступных ядер:"
    echo "  update-kernel --list"
    echo ""
    echo "================================================================"
    echo "        СКРИПТ РАЗРАБОТАН ДЛЯ АВТОМАТИЗАЦИИ УСТАНОВКИ ЯДЕР"
    echo "================================================================"
}
